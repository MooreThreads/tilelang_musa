...
TL_DEVICE fp8_e4_2_t cvt_half2_to_fp8_e4_2(half2 in) {
  const half *lanes = reinterpret_cast<const half *>(&in);
  return make_fp8_e4_2_t(fp8_e4_t(lanes[0]), fp8_e4_t(lanes[1]));
}

TL_DEVICE fp8_e4_4_t cvt_half4_to_fp8_e4_4(__half4 in) {
  const half *lanes = reinterpret_cast<const half *>(&in);
  return make_fp8_e4_4_t(fp8_e4_t(lanes[0]), fp8_e4_t(lanes[1]),
                         fp8_e4_t(lanes[2]), fp8_e4_t(lanes[3]));
}

TL_DEVICE half2 cvt_fp8_e4_2_to_half2(fp8_e4_2_t in) {
  return make_half2_from_halves(static_cast<half>(in.x),
                                static_cast<half>(in.y));
}

TL_DEVICE __half4 cvt_fp8_e4_4_to_half4(fp8_e4_4_t in) {
  return make_half4_from_halves(
      static_cast<half>(in.x), static_cast<half>(in.y), static_cast<half>(in.z),
      static_cast<half>(in.w));
}

TL_DEVICE float2 cvt_fp8_e4_2_to_float2(fp8_e4_2_t in) {
  return __half22float2(cvt_fp8_e4_2_to_half2(in));
}

TL_DEVICE float4 cvt_fp8_e4_4_to_float4(fp8_e4_4_t in) {
  return __half42float4(cvt_fp8_e4_4_to_half4(in));
}

TL_DEVICE fp8_e5_2_t cvt_half2_to_fp8_e5_2(half2 in) {
  const half *lanes = reinterpret_cast<const half *>(&in);
  return make_fp8_e5_2_t(fp8_e5_t(lanes[0]), fp8_e5_t(lanes[1]));
}

TL_DEVICE fp8_e5_4_t cvt_half4_to_fp8_e5_4(__half4 in) {
  const half *lanes = reinterpret_cast<const half *>(&in);
  return make_fp8_e5_4_t(fp8_e5_t(lanes[0]), fp8_e5_t(lanes[1]),
                         fp8_e5_t(lanes[2]), fp8_e5_t(lanes[3]));
}

TL_DEVICE half2 cvt_fp8_e5_2_to_half2(fp8_e5_2_t in) {
  return make_half2_from_halves(static_cast<half>(in.x),
                                static_cast<half>(in.y));
}

TL_DEVICE __half4 cvt_fp8_e5_4_to_half4(fp8_e5_4_t in) {
  return make_half4_from_halves(
      static_cast<half>(in.x), static_cast<half>(in.y), static_cast<half>(in.z),
      static_cast<half>(in.w));
}

TL_DEVICE float2 cvt_fp8_e5_2_to_float2(fp8_e5_2_t in) {
  return __half22float2(cvt_fp8_e5_2_to_half2(in));
}

TL_DEVICE float4 cvt_fp8_e5_4_to_float4(fp8_e5_4_t in) {
  return __half42float4(cvt_fp8_e5_4_to_half4(in));
}
