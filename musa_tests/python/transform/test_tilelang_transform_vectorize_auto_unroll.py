...
def _get_outer_loop_kind(stmt) -> tir.ForKind:
    ...
    def visit(node):
        nonlocal outer_kind
        if isinstance(node, tir.For) and isinstance(node.body, tir.For) and node.body.kind == tir.ForKind.VECTORIZED:
...
def _legalize_vectorized_loop(disable_auto_unroll: bool) -> tir.ForKind:
    ...
    with tvm.transform.PassContext(config=config), auto_target:
