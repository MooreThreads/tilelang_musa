...
def get_kernel_callsite_source(source):
    """Only keep kernel body so helper declarations do not satisfy callsite checks."""
    match = re.search(r'extern "C" __global__ void [A-Za-z_][A-Za-z0-9_]*\(', source)
    if match is None:
        return source
...
def check_fastmath_usage(source, mathop_name, expect_fastmath=False):
    ...
    source = get_kernel_callsite_source(source)
...
def run_fastmath_mathop_test(mathop_name,
    ...
    expect_fastmath = musa_mathop_name not in {"exp10", "log10"}
    check_fastmath_usage(
        source_fastmath, musa_mathop_name, expect_fastmath=expect_fastmath)
    ...
    if musa_mathop_name in ["sqrt", "rsqrt", "log", "log2", "log10"]:
        a = torch.abs(a) + 0.1
    elif musa_mathop_name == "exp10":
        # Keep reference range stable for fp16/fp32 comparison.
    ...
    if musa_mathop_name == "exp":
        ...
        expected = torch.pow(torch.full_like(a, 10.0), a)
    elif musa_mathop_name == "log":
        expected = torch.log(a)
    elif musa_mathop_name == "log10":
