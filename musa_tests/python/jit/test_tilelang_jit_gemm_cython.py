...
def run_cython_kernel_do_bench(M,
    ...
    ctypes_matmul_kernel = None
    try:
        ctypes_matmul_kernel = tilelang.compile(program, execution_backend="ctypes")
    except ValueError:
        # MUSA v0.1.8 only supports cython/tvm_ffi execution backend.
        pass

    cython_profiler = cython_matmul_kernel.get_profiler()
    ctypes_profiler = ctypes_matmul_kernel.get_profiler() if ctypes_matmul_kernel else None
    ...
    if ctypes_profiler is not None:
...
def run_cython_dynamic_shape(M,
    ...
    if dtypeAccum in ("float16", "bfloat16"):
        tensor_ref_c = tilelang.testing.matmul_naive(
            tensor_a, tensor_b, getattr(torch, dtypeAccum), out_dtype)
    else:
